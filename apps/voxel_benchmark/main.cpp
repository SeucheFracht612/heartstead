...
struct Options {
    benchmark::VoxelStorageBenchmarkConfig benchmark;
...
[[nodiscard]] core::Result<Options> parse_options(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        const auto next = [&]() -> core::Result<std::string_view> {
            if (index + 1 >= argc) {
                return core::Result<std::string_view>::failure(
                    "voxel_benchmark.missing_value", std::string(argument) + " requires a value");
            }
        ...
        if (argument == "--help" || argument == "-h") {
            ...
            if (value.value() != "all") {
                ...
                if (!parsed) {
                    return core::Result<Options>::failure(
                        "voxel_benchmark.invalid_corpus",
            ...
            if (value.value() != "all") {
                const auto parsed = parse_unsigned<std::uint16_t>(value.value());
                if (!parsed || (*parsed != 16 && *parsed != 32)) {
                    return core::Result<Options>::failure("voxel_benchmark.invalid_edge",
                                                          "--edge must be 16, 32, or all");
                }
                options.benchmark.edge_lengths = {*parsed};
            }
            ...
            if (!parsed) {
                return core::Result<Options>::failure("voxel_benchmark.invalid_seed",
                                                      "--seed must be an unsigned integer");
            }
            options.benchmark.seed = *parsed;
        } else if (argument == "--warmup" || argument == "--repetitions" ||
                   argument == "--iterations" || argument == "--random-edits" ||
                   argument == "--growth-edits") {
            auto value = next();
            if (!value) {
                return core::Result<Options>::failure(value.error().code, value.error().message);
            }
            ...
            if (!parsed || (*parsed == 0 && argument != "--warmup")) {
                return core::Result<Options>::failure("voxel_benchmark.invalid_count",
                                                      std::string(argument) +
            ...
            if (argument == "--warmup") {
                ...
                options.benchmark.iterations = *parsed;
            } else if (argument == "--random-edits") {
                options.benchmark.random_edits_per_iteration = *parsed;
            } else {
                options.benchmark.palette_growth_edits = *parsed;
            }
            ...
            return core::Result<Options>::failure("voxel_benchmark.unknown_option",
                                                  "unknown voxel benchmark option: " +
                                                      std::string(argument));
        }
    }
...
void print_usage(std::ostream& output) {
    output << "usage: heartstead_voxel_benchmark [options]\n"
              "  --corpus NAME       Corpus name or all (default all)\n"
              "  --edge N            Section edge 16, 32, or all (default all)\n"
              "  --seed N            Deterministic corpus/edit seed\n"
              "  --warmup N          Unmeasured repetitions (default 2; zero allowed)\n"
              "  --repetitions N     Retained raw samples per case (default 9)\n"
              "  --iterations N      Whole-section iterations per sample (default 8)\n"
              "  --random-edits N    Existing-palette edits per iteration (default 256)\n"
              "  --growth-edits N    Novel palette edits per sample (default 64)\n"
...
[[nodiscard]] int run(const Options& options) {
    ...
    std::cerr << "voxel_benchmark.unoptimized_build: rebuild with default-release or an "
                 "optimized profiling configuration\n";
    return 2;
#else
    auto report = benchmark::run_voxel_storage_benchmark(options.benchmark);
    if (!report) {

    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--help" || argument == "-h") {
            ...
            options.benchmark.enforce_gates = true;
                   ...
                   argument == "--steady-ticks" || argument == "--warmup-timeout-ticks" ||
            ...
            if (argument == "--spread-chunks") {
                ...
                options.benchmark.traversal_stride_chunks = *parsed;
                   ...
                   argument == "--tick-max-ms" || argument == "--shared-reuse" ||
                   argument == "--disjoint-reuse") {
            auto value = next();
            if (!value) {
                return core::Result<Options>::failure(value.error().code, value.error().message);
            }
...
void print_usage(std::ostream& output) {
    output << "usage: heartstead_multiplayer_chunk_subscription_benchmark [options]\n"

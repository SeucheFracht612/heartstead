...
struct MultiplayerNetworkImpairmentBenchmarkConfig {
    ...
    std::uint32_t maximum_hard_correction_count = 1;
    double maximum_correction_distance_m = 1.0;
    std::uint64_t maximum_average_server_to_client_bytes_per_second = 64U * 1024U;
    std::uint64_t maximum_rolling_one_second_server_to_client_bytes = 256U * 1024U;
    std::uint32_t minimum_impairment_eligible_unreliable_message_count = 1;
    std::uint32_t minimum_simulated_unreliable_drop_count = 1;
    std::uint32_t maximum_pending_impaired_message_count = 128;
...
struct MultiplayerNetworkImpairmentBenchmarkSummary {
    ...
    std::uint64_t final_acknowledged_input_sequence = 0;
    bool client_connected = false;
};

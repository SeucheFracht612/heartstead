...
inline constexpr std::uint32_t no_terrain_texture_asset =
...
struct TerrainVoxelMaterialAsset {
    ...
    std::uint32_t side_texture = no_terrain_texture_asset;
    std::uint32_t top_texture = no_terrain_texture_asset;
    std::uint32_t bottom_texture = no_terrain_texture_asset;
...
struct TerrainMaterialAssetSet {
    ...
    [[nodiscard]] const TerrainVoxelMaterialAsset*
    find(std::uint16_t voxel_type) const noexcept;

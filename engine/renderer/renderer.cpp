...
core::Status Renderer::synchronize_chunks(world::WorldState& world, const RenderCamera& camera) {
    ...
    auto status = core::Status::ok();
    {
        ...
        if (status && far_terrain_renderer_ != nullptr) {
            ...
            if (surface_revision != far_terrain_world_surface_.revision()) {
                invalidated_regions = far_terrain_world_surface_.synchronize(world, surface_revision);
            }
            const FarTerrainSurfaceSampler sampler = [this](double x, double z,
                                                            FarTerrainDomain domain) {
                return far_terrain_world_surface_.sample(x, z, domain);
            };
...
void Renderer::update_frontend_stats(std::size_t loaded_chunk_count) noexcept {
    ...
    if (far_terrain_renderer_ != nullptr) {
        ...
        stats_.far_terrain_stale_resident_patches = saturating_u32(far.stale_resident_patches);
        stats_.far_terrain_pending_mid_updates = saturating_u32(far.pending_mid_updates);
        stats_.far_terrain_pending_far_updates = saturating_u32(far.pending_far_updates);
        stats_.far_terrain_replaced_patches = saturating_u32(far.replaced_patches);
        ...
        stats_.far_terrain_total_invalidated_patches = far.total_invalidated_patches;
        stats_.far_terrain_total_coalesced_invalidations = far.total_coalesced_invalidations;
        ...
        stats_.far_terrain_stale_resident_patches = 0;
        stats_.far_terrain_pending_mid_updates = 0;
        stats_.far_terrain_pending_far_updates = 0;
        stats_.far_terrain_replaced_patches = 0;
        ...
        stats_.far_terrain_total_invalidated_patches = 0;
        stats_.far_terrain_total_coalesced_invalidations = 0;

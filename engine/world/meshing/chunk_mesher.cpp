...
core::Result<ChunkMesh> build_snapshot_surface_mesh(const ChunkNeighborhoodSnapshot& neighborhood,
    ...
    const auto emit_cell = [&mesh, &neighborhood, &render_table, specialized_only](
                               std::uint16_t x, std::uint16_t y, std::uint16_t z) -> core::Status {
        ...
        if (block->render_phase == MeshingRenderPhase::fluid) {
            ...
            return add_fluid_cell(mesh, fluid_query, {x, y, z}, cell,
        ...
        if (block->geometry == MeshingGeometryKind::cross_plane) {
        ...
        for (const auto& triangle : block->triangles) {
            add_authored_triangle(
                ...
                block->material_index == 0 ? cell.type : block->material_index, block->render_phase,
                [&neighborhood, &render_table](std::int32_t query_x, std::int32_t query_y,
                                               std::int32_t query_z) {
                    const auto queried =
                        query_cell(neighborhood, render_table, query_x, query_y, query_z);
    ...
    if (neighborhood.center_occupancy.full()) {
        for (std::uint16_t z = 0; z < VoxelChunk::edge_length; ++z) {
            for (std::uint16_t y = 0; y < VoxelChunk::edge_length; ++y) {
                for (std::uint16_t x = 0; x < VoxelChunk::edge_length; ++x) {
                    status = emit_cell(x, y, z);
                    if (!status) {
                        return core::Result<ChunkMesh>::failure(status.error().code,

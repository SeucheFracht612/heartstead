...
steady state. Three clean Release processes passed every gate: median server tick P95/P99 was
2.505/4.747 ms, cluster/spread convergence was 10 ticks, traversal convergence was 9 ticks, forced
reliable backlog recovery was 1 tick, shared/disjoint encode reuse was 8/1, peak wire traffic was
9,327 bytes/client/tick, codec overshoot was 27 us median, and all 112 cross-region exclusions were
verified. Runtime adoption plus clean-host spread/convergence/traversal and P99 evidence is therefore
implemented. Committed voxel event batches and typed deltas now share an owner-only chunk routing
scope derived from exact-current client publications. Focused two-client coverage proves one near
delivery, one far exclusion, absence of both payload types at the far client, and exact recovery by
a later full snapshot; relevance reports and Tracy retain delivered/filtered spatial pairs.
Hot-region edit P99, impaired-network P99, game-specific temporal aggregation, and long-soak
evidence remain before the M6 scale milestone is accepted.

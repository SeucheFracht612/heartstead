...
| Versioned chunk pipeline | An owner-thread ledger now separates content, light, mesh, collision, persistence, and replication request/output revisions and states. Save/replication and mesh/GPU publication are ticket-validated across edit and reload races. | Carry stage tickets through light and collision publication, then calibrate stale-work amplification. |

...
| Versioned chunk pipeline | An owner-thread ledger now separates content, light, mesh, collision, persistence, and replication request/output revisions and states. Save/replication publication is ticket-validated across edit and reload races. | Carry stage tickets through light, mesh, collision, and GPU-residency publication, then calibrate stale-work amplification. |
